...
async def get_menu_item_ingredients(item_id: int, db: Session = Depends(get_db)) -> list[MenuItemIngredientOut]:
    ingredients = await asyncio.to_thread(ingredients_service.get_ingredients_by_item_id, item_id, db)
    if not ingredients:
...
async def create_menu_item_ingredient(item_id: int, 
    ...
    ingredient_obj = await asyncio.to_thread(ingredients_service.create_menu_item_ingredient, item_id, ingredient, db)
...
async def delete_menu_item_ingredient(item_id: int, 
    ...
    await asyncio.to_thread(ingredients_service.delete_menu_item_ingredient, item_id, ingredient_id, db)
    return None

#Получение всех ингредиентов
@router.get("/", response_model=list[MenuItemIngredientOut])
async def get_all_menu_item_ingredients(db: Session = Depends(get_db)) -> list[MenuItemIngredientOut]:
    return await asyncio.to_thread(ingredients_service.get_all_menu_item_ingredients, db)