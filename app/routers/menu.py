...
async def read_all_menu_items(db: Session = Depends(get_db)) -> List[MenuItem]:
    return await asyncio.to_thread(service.get_all_menu_items, db)

#Получение позиции меню по id
@router.get("/{item_id}", response_model=schemas.MenuItemOut)
async def read_menu_item(item_id: int, db: Session = Depends(get_db)) -> MenuItem:
    item = await asyncio.to_thread(service.get_menu_item_by_id, item_id, db)
    if not item:
...
async def create_menu_item(item: schemas.MenuItemCreate, 
    ...
    menu_item = await asyncio.to_thread(service.create_menu_item, item, db)
...
async def update_menu_item(item_id: int, 
    ...
    if current_user.role != "Admin":
        raise HTTPException(status_code=403, detail="Только админы могут изменять меню")
    menu_item = await asyncio.to_thread(service.update_menu_item, item_id, item, db)
    if not menu_item:
        raise HTTPException(status_code=404, detail="Позиция меню не найдена")
    await manager.broadcast({
        "type": "menu_update",
...
async def delete_menu_item(item_id: int, 
    ...
    if current_user.role != "Admin":
        raise HTTPException(status_code=403, detail="Только админы могут изменять меню")
    result = await asyncio.to_thread(service.delete_menu_item, item_id, db)
    if not result:
        raise HTTPException(status_code=404, detail="Позиция меню не найдена")
    await manager.broadcast({
        "type": "menu_delete",
