...
async def create_order(order_data: SupplyOrderCreate, 
    ...
    return await supply_service.create_supply_order(db, order_data)
...
async def get_all_orders(db: AsyncSession = Depends(get_db),
    ...
    return await supply_service.get_all_orders(db)

# Получение просроченых поставок
@router.get("/overdue", response_model=List[SupplyOrderOut])
async def get_overdue_orders(db: AsyncSession = Depends(get_db),
                             current_user: User = Depends(get_current_user)) -> List[SupplyOrder]:
    if current_user.role != "Admin":
        raise HTTPException(status_code=403, detail="Доступ запрещен")
    return await supply_service.get_overdue_orders(db)
...
async def get_order(order_id: int, 
    ...
    if current_user.role != "Admin":
        raise HTTPException(status_code=403, detail="Доступ запрещен")
    order = await supply_service.get_order_by_id(db, order_id)
    if not order:
...
async def update_order(order_id: int, 
    ...
    updated = await supply_service.update_order(db, order_id, updates)
    db_order = await supply_service.get_order_by_id(db, order_id)
...
async def delete_order(order_id: int, 
    ...
    if current_user.role != "Admin":
        raise HTTPException(status_code=403, detail="Доступ запрещен")
    success = await supply_service.delete_order(db, order_id)
    if not success:
        raise HTTPException(status_code=404, detail="Поставка не найдена")