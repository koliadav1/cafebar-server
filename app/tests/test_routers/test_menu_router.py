...
class TestMenuRouter:
    ...
    def test_get_all_menu_items_success(self, client, test_db):
    ...
    def test_get_menu_item_by_id_success(self, client, test_db):
        from app.models.menu_items import MenuCategory
    ...
    def test_create_menu_item_as_admin(self, admin_client, sample_menu_item_data, sample_menu_item, mock_websocket_manager):
        with patch('app.routers.menu.manager.broadcast', mock_websocket_manager.broadcast), \
            ...
            response = admin_client.post("/menu/", json=sample_menu_item_data)
            assert response.status_code == 201
            mock_websocket_manager.broadcast.assert_called_once()

    # Тест создания позиции меню без прав админа
    def test_create_menu_item_as_non_admin(self, authenticated_client, sample_menu_item_data):
        response = authenticated_client.post("/menu/", json=sample_menu_item_data)
        
        assert response.status_code == 403
        assert "Только админы могут изменять меню" in response.json()["detail"]

    # Тест обновления позиции меню
    def test_update_menu_item_success(self, admin_client, sample_menu_item, mock_websocket_manager):
        ...
        with patch('app.routers.menu.manager.broadcast', mock_websocket_manager.broadcast), \
            ...
            response = admin_client.put(f"/menu/{sample_menu_item.item_id}", json=update_data)
            
            assert response.status_code == 200
            mock_websocket_manager.broadcast.assert_called()

    # Тест удаления позиции меню
    def test_delete_menu_item_success(self, admin_client, mock_websocket_manager):
        with patch('app.routers.menu.manager.broadcast', mock_websocket_manager.broadcast), \
            patch('app.services.menu_service.delete_menu_item', return_value={"detail": "позиция удалена"}):
            
            response = admin_client.delete("/menu/1")
