...
class TestIngredientsRouter:
    ...
    def test_get_menu_item_ingredients_success(self, client, test_db, sample_ingredient):
        ...
        response = client.get("/menu-item-ingredients/1")
    ...
    def test_get_menu_item_ingredients_not_found(self, client):
        response = client.get("/menu-item-ingredients/999")
    ...
    def test_create_menu_item_ingredient_as_admin(self, admin_client, test_db):
        """Тест создания связи ингредиента с позицией меню админом"""
        ingredient_data = {
            "ingredient": {
        ...
        response = admin_client.post("/menu-item-ingredients/1", json=ingredient_data)
        ...
        # Дополнительная проверка что ингредиент создан в БД
        from app.models.ingredients import Ingredient
        db_ingredient = test_db.query(Ingredient).filter(Ingredient.name == "Новый ингредиент").first()
        assert db_ingredient is not None

    # Тест создания связи ингредиента без прав админа
    def test_create_menu_item_ingredient_as_non_admin(self, authenticated_client):
        ingredient_data = {
            "ingredient": {
                "name": "Новый ингредиент",
                "unit": "кг",
        ...
        response = authenticated_client.post("/menu-item-ingredients/1", json=ingredient_data)

        assert response.status_code == 403
        assert "Только админы могут добавлять ингредиенты" in response.json()["detail"]

    # Тест удаления связи ингредиента админом
    def test_delete_menu_item_ingredient_as_admin(self, admin_client):
        with patch('app.services.ingredients_service.delete_menu_item_ingredient', 
            ...
            response = admin_client.delete("/menu-item-ingredients/1/1")

            assert response.status_code == 204

    # Тест получения всех связей ингредиентов
    def test_get_all_menu_item_ingredients(self, client, test_db):
        response = client.get("/menu-item-ingredients/")
