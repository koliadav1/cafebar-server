...
class TestReviewsRouter:
    ...
    def test_get_all_reviews_as_client(self, authenticated_client, test_db):
        review1 = Review(user_id=1, order_id=1, rating=5)
        review2 = Review(user_id=2, order_id=2, rating=4)
        test_db.add_all([review1, review2])
        test_db.commit()

        response = authenticated_client.get("/reviews/")
    ...
    def test_get_all_reviews_unauthorized(self, authenticated_client):
    ...
    def test_create_review_as_client(self, authenticated_client, sample_review_data, sample_review):
        with patch('app.services.review_service.create_review', return_value=sample_review):
            response = authenticated_client.post("/reviews/", json=sample_review_data)
            assert response.status_code == 200
            data = response.json()
    ...
    def test_create_review_as_non_client(self, admin_client, sample_review_data):
        response = admin_client.post("/reviews/", json=sample_review_data)
        assert response.status_code == 403
        assert "Только клиенты могут оставлять отзывы" in response.json()["detail"]

    # Тест получения отзывов пользователя админом
    def test_get_reviews_by_user_as_admin(self, admin_client, test_db):
        review = Review(user_id=1, order_id=1, rating=5)
        test_db.add(review)
        test_db.commit()

        response = admin_client.get("/reviews/user/1")
    ...
    def test_get_reviews_by_user_as_non_admin(self, authenticated_client):
        response = authenticated_client.get("/reviews/user/1")
        assert response.status_code == 403
        assert "Доступ запрещен" in response.json()["detail"]

    # Тест получения отзыва по ID
    def test_get_review_by_id(self, client, test_db):
    ...
    def test_update_review_as_owner(self, authenticated_client, sample_review):
        with patch('app.services.review_service.update_review') as mock_update:
            ...
            response = authenticated_client.put(f"/reviews/{sample_review.review_id}", json=update_data)
            assert response.status_code == 200
            data = response.json()
    ...
    def test_update_review_as_non_owner(self, authenticated_client):
        ...
        with patch('app.services.review_service.get_review_by_id', return_value=mock_review):
            ...
            response = authenticated_client.put("/reviews/1", json=update_data)

            assert response.status_code == 403
            assert "только свои отзывы" in response.json()["detail"]

    # Тест удаления отзыва админом
    def test_delete_review_as_admin(self, admin_client, sample_review):
        with patch('app.services.review_service.delete_review') as mock_delete:
            mock_delete.return_value = sample_review
            response = admin_client.delete(f"/reviews/{sample_review.review_id}")
            assert response.status_code == 200

    # Тест удаления отзыва владельцем
    def test_delete_review_as_owner(self, authenticated_client, sample_review):
        with patch('app.services.review_service.delete_review', return_value=sample_review):
            response = authenticated_client.delete(f"/reviews/{sample_review.review_id}")
            assert response.status_code == 200
            data = response.json()
            assert data["review_id"] == sample_review.review_id

    # Тест ответа на отзыв админом
    def test_respond_to_review_as_admin(self, admin_client, sample_review):
        with patch('app.services.review_service.respond_to_review') as mock_respond:
            ...
            response = admin_client.post(f"/reviews/{sample_review.review_id}/response", json=response_data)
    ...
    def test_respond_to_review_as_non_admin(self, authenticated_client):
        response_data = {
            "admin_response": "Спасибо за отзыв!"
        }

        response = authenticated_client.post("/reviews/1/response", json=response_data)
