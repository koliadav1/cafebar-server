...
class TestBookingRouter:
    ...
    def test_create_booking_as_client(self, authenticated_client, sample_booking_data, sample_booking):
        with patch('app.services.booking_service.create_booking', return_value=sample_booking):
            response = authenticated_client.post("/bookings/", json=sample_booking_data)
    ...
    def test_create_booking_unauthorized_role(self, authenticated_client):
    ...
    def test_get_bookings_by_user(self, client, sample_booking):
        response = client.get(f"/bookings/?user_id={sample_booking.user_id}")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
    ...
    def test_get_bookings_by_status(self, client, sample_booking):
        response = client.get(f"/bookings/?status={sample_booking.status.value}")
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
    ...
    def test_get_booking_by_id(self, client, sample_booking):
        response = client.get(f"/bookings/{sample_booking.booking_id}")
        assert response.status_code == 200
        data = response.json()
    ...
    def test_get_booking_by_id_not_found(self, client):
    ...
    def test_update_booking_as_client(self, authenticated_client, sample_booking):
        with patch('app.services.booking_service.update_booking') as mock_update:
            ...
            response = authenticated_client.put(f"/bookings/{sample_booking.booking_id}", json=update_data)
            assert response.status_code == 200
            data = response.json()
            assert data["customer_name"] == "Обновленное Имя"

    # Тест удаления бронирования админом
    def test_delete_booking_as_admin(self, admin_client, sample_booking):
        with patch('app.services.booking_service.delete_booking', return_value=True):
            response = admin_client.delete(f"/bookings/{sample_booking.booking_id}")
            assert response.status_code == 200
