...
class TestOrderRouter:
    ...
    def test_get_orders_as_client(self, authenticated_client):
    ...
    def test_get_orders_as_staff(self, authenticated_client):
    ...
    def test_get_order_by_id_as_staff(self, admin_client, sample_order):
        sample_order.items = []
        with patch('app.services.order_service.get_order_by_id', return_value=sample_order):
            response = admin_client.get(f"/orders/{sample_order.order_id}")
            assert response.status_code == 200
            data = response.json()
    ...
    def test_get_order_by_id_as_client_denied(self, authenticated_client):
        authenticated_client.user.role = "Client"
    ...
    def test_create_order_as_client(self, authenticated_client, sample_order_data, sample_order):
        with patch('app.services.order_service.create_order', return_value=sample_order):
            response = authenticated_client.post("/orders/", json=sample_order_data)
            assert response.status_code == 200

    # Тест создания заказа официантом
    def test_create_order_as_waiter(self, authenticated_client, sample_order_data, sample_order):
        authenticated_client.user.role = "Waiter"
        with patch('app.services.order_service.create_order', return_value=sample_order):
            response = authenticated_client.post("/orders/", json=sample_order_data)
            assert response.status_code == 200
    ...
    def test_create_order_denied(self, authenticated_client, sample_order_data):
        authenticated_client.user.role = "Cook"
        response = authenticated_client.post("/orders/", json=sample_order_data)
        assert response.status_code == 403

    # Тест обновления статуса заказа клиентом
    def test_update_order_status_as_client(self, authenticated_client, sample_order):
        with patch('app.services.order_service.get_order_by_id', return_value=sample_order), \
            patch('app.services.order_service.update_order_status', return_value=sample_order):
            response = authenticated_client.patch("/orders/1/status?status=Cancelled")
            assert response.status_code == 200

    # Тест самоназначения на заказ
    def test_assign_self_to_order(self, authenticated_client, sample_order):
        authenticated_client.user.role = "Waiter"
        with patch('app.services.order_service.get_order_by_id', return_value=sample_order), \
            patch('app.services.order_service.assign_staff_to_order', return_value=sample_order):
            
            response = authenticated_client.patch("/orders/1/assign")
            assert response.status_code == 200