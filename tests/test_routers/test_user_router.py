...
class TestUserRouter:
    ...
    @pytest.mark.asyncio
    async def test_get_users_as_non_admin(self, authenticated_client):
    ...
    @pytest.mark.asyncio
    async def test_get_my_user_data(self, authenticated_client):
    ...
    @pytest.mark.asyncio
    async def test_create_user_success(self, client, sample_user_data, sample_user):
    ...
    @pytest.mark.asyncio
    async def test_delete_own_account(self, authenticated_client):
    ...
    @pytest.mark.asyncio
    async def test_delete_user_as_non_admin(self, authenticated_client):
    ...
    @pytest.mark.asyncio
    async def test_get_users_by_role_as_admin(self, admin_client):
        with patch('app.services.user_service.get_users_by_role', return_value=[]):
            response = await admin_client.get("/users/role/Client")
            assert response.status_code == 200
            assert isinstance(response.json(), list)

    # Тест изменения пароля
    @pytest.mark.asyncio
    async def test_change_password(self, authenticated_client, sample_user):
        password_data = {
        ...
        with patch('app.services.user_service.update_user_password', return_value=sample_user):
            response = await authenticated_client.put("/users/me/password", json=password_data)
    ...
    @pytest.mark.asyncio
    async def test_update_user_data(self, authenticated_client, sample_user):
